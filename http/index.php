...
function webcamDisplay(){
	if (!isset($_SESSION["webcam"])){		
		...
		$_SESSION["webcam"] = $xml->DisplayWebcam;
	}
	if ($_SESSION["webcam"]==0){
		return ' ';
	}else{
		return '<p class="webcam"><img class="oeil" 
src="http://'.$_SERVER['SERVER_ADDR'].':6881" /></p>';
	}
}
  ?>
  <div class="row">
	...
	if (isset($dev['Temperature'])) {
	  ...
	  $hrate = number_format((float)$hrate, 2, '.', '');
          $hrate = $hrate . " GH/s";
          	...
          	if ($dev['MHSav'] > 999999999){ $hrate = $dev['MHSav'] / 1000000000; $hrate = $hrate . " PH/s"; }
        }else{
...
// class="success" error warning info
  ...
  foreach ($pools as $pool) {
	...
	if($pool['Status'] != "Alive"){ $colorStatusPool = "error_red";}else{ $colorStatusPool= "alive"; }
	$table = $table . "
    <td class='text-left'>" . $poolURL[1] . "</td>
    <td class='text-left ellipsis'>" . $pool['User'] . "</td>
    <td class='text-left $colorStatusPool'>" . $pool['Status'] . "</td>
    <td>" . $pool['Priority'] . "</td>
    <td>" . $pool['Getworks'] . "</td>
    <td>" . $pool['Accepted'] . "</td>
    <td>" . $pool['Rejected'] . "</td>
